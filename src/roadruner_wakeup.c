...
int8_t runner_wakeup_int(void){
    if (!device_is_ready(dev_op1.port)) {
        LOG_ERR("RR Wakeup - GPIO initialization for waking up RoadRunner is not successful!");
	    return (1);
    }
    ret_SW = gpio_pin_configure_dt(&dev_op1, GPIO_OUTPUT_ACTIVE);
    if (ret_SW < 0) {
	    LOG_ERR("RR wakeup - Pin Configuration for waking up RoadRunner is not successful!");
        return (1);
    }
    return 0;
}
